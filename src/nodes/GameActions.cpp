...
GameActions::GameActions(GameTreeNode::PokerActions action, double amount) {
    ...
    if (action == GameTreeNode::PokerActions::RAISE || action == GameTreeNode::PokerActions::BET) {
        if (amount != -1) throw runtime_error(fmt::format("raise/bet amount should be -1, find {}",amount));
    } else {
        if (amount == -1) throw runtime_error(fmt::format("check/fold/call amount should not be -1, find {}",amount));
    }
    this->amount = amount;
}
