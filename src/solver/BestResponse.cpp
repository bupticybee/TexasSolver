...
float BestResponse::printExploitability(shared_ptr<GameTreeNode> root, int iterationCount, float initial_pot,
    ...
    for (int player_id = 0; player_id < this->player_number; player_id++) {
        vector<float> reach_prob_player(private_combos[player_id].size());
...
float BestResponse::getBestReponseEv(shared_ptr<GameTreeNode> node, int player, vector<vector<float>> reach_probs,
    ...
    vector<float> private_cards_evs = bestResponse(node, player, reach_probs, initialBoard);
...
vector<float> BestResponse::bestResponse(shared_ptr<GameTreeNode> node, int player, vector<vector<float>> reach_probs,
                                         uint64_t board) {
...
vector<float>
BestResponse::chanceBestReponse(shared_ptr<ChanceNode> node, int player, vector<vector<float>> reach_probs,
    ...
    for(int card = 0;card < node->getCards().size();card ++){
        ...
        vector<PrivateCards> playerPrivateCard = this->pcm.getPreflopCards(player);//this.getPlayerPrivateCard(player);
        vector<PrivateCards> oppoPrivateCards = this->pcm.getPreflopCards(1 - player);
...
vector<float>
BestResponse::actionBestResponse(shared_ptr<ActionNode> node, int player, vector<vector<float>> reach_probs,
                                 uint64_t board) {
    if(player == node->getPlayer()){
        ...
        vector<float> my_exploitability;
        for(shared_ptr<GameTreeNode> one_node:node->getChildrens()){
            vector<float> node_ev = this->bestResponse(one_node,player,reach_probs,board);
            if(my_exploitability.empty()){
        ...
        for(int action_ind = 0;action_ind < node->getChildrens().size();action_ind ++){
            ...
            for(int i = 0;i < this->player_number;i ++){
                if(i == node->getPlayer()) {
                    ...
                    vector<float> next_reach_probs_current_player(private_combo_numbers);
            ...
            vector<float> action_payoffs = this->bestResponse(one_child,player,next_reach_probs,board);
...
vector<float>
BestResponse::terminalBestReponse(shared_ptr<TerminalNode> node, int player, vector<vector<float>> reach_probs,
                                  uint64_t board) {
    uint64_t board_long = board;
    int oppo = 1 - player;
    vector<RiverCombs> player_combs = this->rrm.getRiverCombos(player,this->pcm.getPreflopCards(player),board);  //this.river_combos[player];
    vector<RiverCombs> oppo_combs = this->rrm.getRiverCombos(1 - player,this->pcm.getPreflopCards(1 - player),board);  //this.river_combos[player];

    float player_payoff = node->get_payoffs()[player];
    vector<float> payoffs(player_hands[player]);
    ...
    vector<float> oppo_reach_prob = reach_probs[1 - player];
...
vector<float>
BestResponse::showdownBestResponse(shared_ptr<ShowdownNode> node, int player, vector<vector<float>> reach_probs,
                                   uint64_t board) {
