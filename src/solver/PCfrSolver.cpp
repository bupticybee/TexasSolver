...
vector<float> PCfrSolver::cfr(int player, shared_ptr<GameTreeNode> node,const vector<int>& reach_inds, const vector<float> &reach_probs, int iter,
    ...
    switch(node->getType()) {
        case GameTreeNode::ACTION: {
            ...
            return actionUtility(player, action_node,reach_inds ,reach_probs, iter, current_board,deal);
        }case GameTreeNode::SHOWDOWN: {
            shared_ptr<ShowdownNode> showdown_node = std::dynamic_pointer_cast<ShowdownNode>(node);
            return showdownUtility(player, showdown_node,reach_inds , reach_probs, iter, current_board,deal);
        }case GameTreeNode::TERMINAL: {
            shared_ptr<TerminalNode> terminal_node = std::dynamic_pointer_cast<TerminalNode>(node);
            return terminalUtility(player, terminal_node,reach_inds , reach_probs, iter, current_board,deal);
        }case GameTreeNode::CHANCE: {
            shared_ptr<ChanceNode> chance_node = std::dynamic_pointer_cast<ChanceNode>(node);
            return chanceUtility(player, chance_node, reach_inds ,reach_probs, iter, current_board,deal);
        }default:
...
vector<float>
PCfrSolver::chanceUtility(int player, shared_ptr<ChanceNode> node,const vector<int>& reach_inds, const vector<float> &reach_probs, int iter,
                         uint64_t current_board,int deal) {
    ...
    for(int valid_ind = 0;valid_ind < valid_cards.size();valid_ind++) {
        ...
        new_reach_probs.reserve(reach_probs.size());
        vector<int> new_reach_index;// = vector<float>(oppoPrivateCards.size());
        new_reach_probs.reserve(reach_inds.size());
        ...
        for (int player_hand:reach_inds) {
            ...
            new_reach_probs.push_back(reach_probs[player_hand] / possible_deals);
            new_reach_index.push_back(player_hand);
        }
#ifdef DEBUG
        ...
        if(this->distributing_task && node->getRound() == this->split_round) {
            ...
            vector<float> child_utility = this->cfr(player, one_child,new_reach_index ,new_reach_probs, iter, new_board_long, new_deal);
...
vector<float>
PCfrSolver::actionUtility(int player, shared_ptr<ActionNode> node,const vector<int>& reach_inds, const vector<float> &reach_probs, int iter,
                         uint64_t current_board,int deal) {
    int oppo = 1 - player;
    ...
    for (int action_id = 0; action_id < actions.size(); action_id++) {
        ...
        if (node_player != player) {
            ...
            new_reach_prob.reserve(reach_probs.size());
            vector<int> new_reach_inds;// = vector<float>(reach_probs.size());
            new_reach_inds.reserve(reach_inds.size());
            for (int hand_id:reach_inds) {
                float strategy_prob = current_strategy[hand_id + action_id * node_player_private_cards.size()];
                float reach_prob_prob = reach_probs[hand_id] * strategy_prob;
                if(reach_prob_prob != 0) {
                    new_reach_prob.push_back(reach_prob_prob);
                    new_reach_inds.push_back(hand_id);
                }
            }
            //#pragma omp task shared(results,action_id)
            results[action_id] = this->cfr(player, children[action_id],new_reach_inds , new_reach_prob, iter,
                                           current_board,deal);
        }else {
            //#pragma omp task shared(results,action_id)
            results[action_id] = this->cfr(player, children[action_id],reach_inds, reach_probs, iter,
                                           current_board,deal);
        }
...
vector<float>
PCfrSolver::showdownUtility(int player, shared_ptr<ShowdownNode> node,const vector<int>& reach_inds, const vector<float> &reach_probs,
    ...
    for(int i = 0;i < player_combs.size();i ++){
        ...
        while (j < oppo_combs.size() && one_player_comb.rank < oppo_combs[j].rank){
            ...
            // TODO now what about this motherfucker?
...
vector<float>
PCfrSolver::terminalUtility(int player, shared_ptr<TerminalNode> node,const vector<int>& reach_inds, const vector<float> &reach_prob, int iter,
                           uint64_t current_board,int deal) {
