...
TEST(TestCase,test_build_tree_and_solve){
    ...
    float big_blind=1;
    float stack=100;
    StreetSetting gbs = StreetSetting(vector<float>{100},vector<float>{100},vector<float>{100},true);
    GameTreeBuildingSettings gtbs = GameTreeBuildingSettings(gbs,gbs,gbs,gbs,gbs,gbs);
    ps.build_game_tree(oop_commit,ip_commit,current_round,raise_limit,small_blind,big_blind,stack,gtbs);
